...
- `skillserver` 提供本地 HTTPS Web 能力，使用自签证书，内部调用 SearXNG 和 Playwright。
...
4. 自动生成 SkillServer 本地自签 TLS 证书，并把信任链打进镜像
5. 构建本地镜像（`hermes-agent:local`、`open-webui:local`、`skillserver:local`）
6. 启动整套容器
7. 依次检查 Dashboard、API Server、Open WebUI、SkillServer、SearXNG 连通性
...
启动阶段会强制走本地 build；本地镜像服务在 Compose 里设置了 `pull_policy: never`，避免把这些本地标签误当成远端仓库去拉取，同时保留 `searxng` 这类远端镜像的正常 pull 行为。
...
`deploy.py` 还会在 `skillserver/certs/` 下自动生成本地开发用 TLS 文件：

- `skillserver-ca.crt` / `skillserver-ca.key`：本地 CA
- `skillserver.crt` / `skillserver.key`：签给 `skillserver`、`localhost`、`127.0.0.1` 的服务端证书
- `skillserver.fullchain.crt`：服务端证书 + CA 链
